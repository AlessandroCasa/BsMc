...
#' @param data data
#' @param min_obj a list with the following object \itemize{
#' \item \verb{minima} the location of the minima points.
#' \item \verb{fmins} the values of the density computed on \verb{minima}.
#' \item \verb{f2mins} the values of the second derivative of the density computed on \verb{minima}.
#' \item \verb{f3mins} the values of the third derivative of the density computed on \verb{minima}.
#' }
#' @param n the sample size
#' @return a value for the modal clustering-oriented optimal bandwidth
#' @references Casa A., Chacón, J.E. and Menardi, G. (2019). Modal clustering asymptotics with applications to bandwidth selection (https://arxiv.org/pdf/1901.07300.pdf).

hab1 <- function(data,min_obj,n) {
  mu2K<-1
  RK1<-1/(4*sqrt(pi))
